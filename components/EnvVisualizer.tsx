...
const EnvVisualizer: React.FC<EnvVisualizerProps> = ({ gameState }) => {
    ...
    // Resolve .env file - check both current directory and project root
    let envFileContent = '';
    let envFileFound = false;

    // First, try project directory (most common location)
    const projectDir = fileSystem?.['project'];
    if (projectDir?.children?.['.env']?.type === 'file') {
        envFileContent = projectDir.children['.env'].content || '';
        envFileFound = true;
    }
    ...
    return (
        <div className="h-full w-full flex flex-col bg-[#0f1115] p-6 space-y-6 overflow-auto custom-scrollbar">
            ...
            {/* .env File Panel */}
            <div className={`bg-[#18181b] border rounded-xl p-4 relative overflow-hidden group transition-all ${envFileFound
                ...
                <h3 className="text-xs font-mono text-cyber-muted mb-3 flex items-center gap-2">
                    <Lock size={12} className={envFileFound ? 'text-orange-400' : ''} />
                    .env FILE CONTENT
                    {envFileFound && (
                        <span className="px-2 py-0.5 bg-orange-500/20 text-orange-400 rounded-full text-[10px] font-bold ml-2" title="Environment secrets stored locally">
                            🔐 SECRET
                        </span>
                    )}
                </h3>
                <div className={`font-mono text-sm bg-black/30 p-3 rounded border min-h-[60px] whitespace-pre-wrap ${envFileFound ? 'border-orange-500/20 text-orange-200' : 'border-white/5 text-orange-200/80'
                    }`}>
                    {envFileContent || <span className="text-white/20 italic">File not found. Use echo "KEY=VALUE" &gt; .env to create</span>}
                </div>
            </div>
